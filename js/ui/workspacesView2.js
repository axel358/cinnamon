const { Cinnamon, Clutter, Gio, GObject, Meta, St } = imports.gi;

const Layout = imports.ui.layout;
const Main = imports.ui.main;
const OverviewControls = imports.ui.coverviewControls;
// const SwipeTracker = imports.ui.swipeTracker;
const Util = imports.misc.util;
const Workspace = imports.ui.workspace2;
const { ThumbnailsBox, MAX_THUMBNAIL_SCALE } = imports.ui.workspaceThumbnail;

var WORKSPACE_SWITCH_TIME = 250;

const MUTTER_SCHEMA = 'org.cinnamon.muffin';
...
var WorkspacesViewBase = GObject.registerClass({
    ...
    _init(monitorIndex, overviewAdjustment) {
        ...
        this._inDrag = false;
        // this._windowDragBeginId = Main.coverview.connect('window-drag-begin', this._dragBegin.bind(this));
        // this._windowDragEndId = Main.coverview.connect('window-drag-end', this._dragEnd.bind(this));
    ...
    _onDestroy() {
        ...
        if (this._windowDragBeginId > 0) {
            Main.coverview.disconnect(this._windowDragBeginId);
            this._windowDragBeginId = 0;
        }
        if (this._windowDragEndId > 0) {
            Main.coverview.disconnect(this._windowDragEndId);
    ...
    vfunc_allocate(box, flags) {
        ...
        for (const child of this)
            child.allocate_available_size(0, 0, box.get_width(), box.get_height(), flags);
    }
...
class WorkspacesView extends WorkspacesViewBase {
    _init(monitorIndex, controls, scrollAdjustment, fitModeAdjustment, overviewAdjustment) {
        ...
        global.log("workspacesview creating new WorkspacesView");
    ...
    _getSpacing(box, fitMode, vertical) {
        ...
        return spacing.clamp(WORKSPACE_MIN_SPACING * scaleFactor,
    ...
    _updateWorkspacesState() {
        ...
        global.log("workspacesView _updateWorkspaceState");
        ...
        //     const scaleProgress = 1 - distanceToCurrentWorkspace.clamp(0, 1);
    ...
    vfunc_allocate(box, flags) {
        ...
        workspaces.forEach(child => {
            ...
            child.allocate_align_fill(box, 0.5, 0.5, false, false, flags);
    ...
    _updateWorkspaces() {
        global.log("workspacesview _updateWorkspaces");
        let workspaceManager = global.workspace_manager;
...
class ExtraWorkspaceView extends WorkspacesViewBase {
    ...
    _updateWorkspaceMode() {
        ...
        const progress = overviewState.clamp(
...
class WorkspacesDisplay extends St.Widget {
    ...
    prepareToEnterOverview() {
        global.log("workspacesView Prepare to enter overview---------------------");
        this.show();
        this._updateWorkspacesViews();

        this._restackedNotifyId =
            Main.coverview.connect('windows-restacked',
                                   this._onRestacked.bind(this));
        if (this._scrollEventId == 0)
            this._scrollEventId = Main.coverview.connect('scroll-event', this._onScrollEvent.bind(this));
    ...
    vfunc_hide() {
        if (this._restackedNotifyId > 0) {
            Main.coverview.disconnect(this._restackedNotifyId);
            this._restackedNotifyId = 0;
        }
        if (this._scrollEventId > 0) {
            Main.coverview.disconnect(this._scrollEventId);
            this._scrollEventId = 0;
        }
        if (this._keyPressEventId > 0) {
    ...
    _updateWorkspacesViews() {
        global.log("workspacesView update workspaces view---------------------");
        for (let i = 0; i < this._workspacesViews.length; i++)
            this._workspacesViews[i].destroy();
        ...
        global.log("workspacesView done update workspaces view---------------------");
    }
