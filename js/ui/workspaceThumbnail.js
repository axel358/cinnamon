...
const { Cinnamon, Clutter, Gio, GLib, GObject, Graphene, Meta, St } = imports.gi;
...
var MUTTER_SCHEMA = 'org.cinnamon.muffin';
...
var ThumbnailsBox = GObject.registerClass({
    ...
    _init(scrollAdjustment, monitorIndex) {
        ...
        this._overviewSignals = [
            Main.coverview.connect('showing',
                () => this._createThumbnails()),
            Main.coverview.connect('hidden',
                () => this._destroyThumbnails()),
            // Main.coverview.connect('item-drag-begin',
            //     () => this._onDragBegin()),
            // Main.coverview.connect('item-drag-end',
            //     () => this._onDragEnd()),
            // Main.coverview.connect('item-drag-cancelled',
            //     () => this._onDragCancelled()),
            // Main.coverview.connect('window-drag-begin',
            //     () => this._onDragBegin()),
            // Main.coverview.connect('window-drag-end',
            //     () => this._onDragEnd()),
            // Main.coverview.connect('window-drag-cancelled',
            //     () => this._onDragCancelled()),
        ];
    ...
    _destroyThumbnails() {
        ...
        if (this._syncStackingId > 0) {
            Main.coverview.disconnect(this._syncStackingId);
            this._syncStackingId = 0;
        }
