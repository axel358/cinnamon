...
function animateAllocation(actor, box, flags) {
...
var WorkspaceLayout = GObject.registerClass({
    ...
    _init(metaWorkspace, monitorIndex, overviewAdjustment) {
        ...
        global.log("workspace2 WorkspaceLayout");
    ...
    vfunc_set_container(container) {
        global.log("vfunc_set container_______________________________________");
    ...
    vfunc_allocate(container, box, flags) {
        ...
        if (this._layoutFrozen && containerAllocationChanged && !Main.coverview.animationInProgress) {
            this._layoutFrozen = false;
        ...
        for (let i = 0; i < nSlots; i++) {
            ...
            if (windowInfo.currentTransition) {
                ...
                child.allocate(child.allocation, flags);
                continue;
            }
            ...
            if (layoutChanged && !Main.coverview.animationInProgress) {
                const transition = animateAllocation(child, childBox, flags);
                if (transition) {
...
class WorkspaceBackground extends St.Widget {
    ...
    _init(monitorIndex) {
        ...
        global.log("workspace2 WorkspaceBackground");
    ...
    _updateBorderRadius() {
        global.log("workspace2 _updateBorderRadius");
        ...
        backgroundContent.rounded_clip_radius =
            Util.lerp(0, cornerRadius, 1);
    }

    _updateRoundedClipBounds() {
        global.log("workspace2 _updateRoundedClipBounds");
        const monitor = Main.layoutManager.monitors[this._monitorIndex];

        global.log(this._workarea);
        global.log("workarea.x " + this._workarea.x);
        global.log("monitor.x " + monitor.x);
    ...
    vfunc_allocate(box, flags) {
        global.log("workspace2 vfunc_allocate");
        ...
        this._bin.allocate(contentBox, flags);
        ...
        this._backgroundGroup.allocate(contentBox, flags);
    }
...
class Workspace extends St.Widget {
    _init(metaWorkspace, monitorIndex, overviewAdjustment) {
        ...
        global.log("worspace2 creating new Workspace");
        ...
        global.log("workspace2 Creating click action");
    ...
    _doAddWindow(metaWin) {
        ...
        //         return GLib.SOURCE_REMOVE;
        //     });
        //     GLib.Source.set_name_by_id(id, '[cinnamon] this._doAddWindow');
        //     return;
        // }
    ...
    _windowEnteredMonitor(metaDisplay, monitorIndex, metaWin) {
        // if (monitorIndex === this.monitorIndex && !Main.coverview.closing)
        //     this._doAddWindow(metaWin);
    }
    ...
    prepareToLeaveOverview() {
        ...
        // this._container.layout_manager.layout_frozen = true;
        this._overviewHiddenId = Main.coverview.connect('hidden', this._doneLeavingOverview.bind(this));
    }
    ...
    _onDestroy() {
        ...
        if (this._overviewHiddenId) {
            Main.coverview.disconnect(this._overviewHiddenId);
    ...
    _addWindowClone(metaWindow) {
        // global.log("workspace2 _addWindowClone");
        ...
        // global.log("workspace2 _addWindowClone done");
