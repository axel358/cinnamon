    ...
    constructor(device, status, aliases) {
        ...
        if (battery_level == UPDeviceLevel.NONE) {
            ...
            statusLabel = new St.Label({
                text: "%s - %d%%".format(status, Math.round(percentage)),
        ...
        let device_box = new St.BoxLayout({
            vertical: false,
            style_class: 'device-box',
            y_align: Clutter.ActorAlign.CENTER
        });

        device_box.add_actor(this._icon);

        this._vbox.add_actor(this.label);
        this._vbox.add_actor(statusLabel);

        this._primaryBox.add_actor(device_box);
